...
5. **Usage tracking** — Token usage is accumulated in memory, keyed by `(api_token, YYYY-MM-DD)`. The in-memory store is reset on service restart.
6. **Usage endpoint** — `GET /usage` is a reserved endpoint served by the proxy itself (not forwarded to Ollama). It returns a JSON summary of accumulated usage, grouped by API token and date.
...
- Thread-safe in-memory counter (mutex-protected map).
...
- [ ] `GET /usage` (with valid auth) returns JSON with per-token, per-day usage totals.
